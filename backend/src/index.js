...
const updateProfile = async (req, res) => {
  try {
    const { profilePic, userId } = req.body;  // Changed to get userId from body
...
const checkAuth = (req, res) => {
  try {
    const { userId } = req.body;  // Changed to get userId from body
    if (!userId) {
      return res.status(401).json({ message: "User ID is required" });
    }
...
const sendMessage = async (req, res) => {
  try {
    const { text, image, userId } = req.body;  // Changed to get userId from body
    const { id: receiverId } = req.params;
...
app.get("/api/messages/users", getUsersForSidebar);
